from typing import Dict, Optional, Union, Callable, Any, Generic, TypeVar, Tuple, List
import sys
...
class IO:
    ...
    def find_args(self, argv: List[str]):
        skip = False
                ...
                for key in self.values:
                    ...
                    if (
                        ...
                        and (
                            ...
                            or arg.split("=", 1)[0] == "--" + self.dash_alias(alias)
                        )
                    ):
    ...
    def gen_io_val(self) -> Dict[str, Any]:
        io_val: Dict[str, Any] = dict()
        for key in self.values:
            alias = self.values[key].alias
            if alias:
                io_val[alias] = self.values[key].value
        return io_val

    def __init__(self, values: Dict[str, IOInput[Any]]):
        self.values = values
