...
namespace BinGenerator {
	...
	namespace IO {
		...
		export namespace Output {
			...
			function getFileOutPath(io: IO.Input.IO, { file }: BinFile) {
				return path.join(
					io.outDir,
			...
			export async function writeBins(
				...
				let done: number = 0;
				await fs.ensureDir(path.dirname(io.outDir));
				return Promise.all(
					binFiles.map(async (binFile) => {
						await writeBin(io, binFile);
						Log.groupLog(
	...
	namespace GenBins {
		...
		namespace Bars {
			...
			export async function create(
				...
				const MultiProgress: typeof multiProgress = (multiProgress as any)(
					ProgressBar
				);
	...
	export async function main() {
		...
		await IO.Output.writeBins(io, bins);
		Log.groupEnd();
